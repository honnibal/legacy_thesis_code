...
PB_PATH = '/home/mhonn/Data/propbank_1'
...
#        self._readLexicon(propbankDir)
    ...
    def entries(self, corpus):
        for f in corpus.children():
            for sentencesAndEntries in propbank.fileEntries(f):
                sentences = sentencesAndEntries.keys()
                sentences.sort()
                for sentence in sentences:
                    entries = sentencesAndEntries[sentence]
    ...
    def fileEntries(self, corpusFile):
        ...
        for line in lines:
            ...
            sentence = corpusFile.child(entry.sentIdx)
            entries.setdefault(sentence, []).append(entry)
        return entries
...
class PredArgStructure:
    ...
    def __str__(self):
        ...
        pieces = [self.path, self.sentIdx, self.rel.sentIdx, self.gold, self.frame, self.morph] + pargStrings
