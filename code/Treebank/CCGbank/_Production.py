...
class _Production(object):
    ...
    def __init__(self, left, right, parent, functorPos = None):
        self.left = CCG.Category(str(left))
        self.right = CCG.Category(str(right))
...
class AdjunctDetermination(Application):
    ...
    def replace(self, new):
        if CCG.isIdentical(new, CCG.NP):
            self.functor = CCG.Category(r'NP[nb]/N')
            return self.left, self.right
        elif new.isAdjunct():
...
if __name__ == '__main__':
    if 1:
        parent = CCG.Category(r'NP\NP')
        cat1 = CCG.Category(r'(S[dc]\NP)^(NP\NP)')
        cat2 = None
        production = Production(cat1, cat2, parent)
        print production
        production.replace(CCG.Category(r'N\N'))
